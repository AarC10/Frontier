...
static VoltageMonitor voltageMonitor(DEVICE_DT_GET(DT_ALIAS(vbat_sensor)),
				     DEVICE_DT_GET(DT_ALIAS(vcc_sensor)));
...
static void voltageThreadEntry(void *, void *, void *) {
	while (true) {
		...
		// TODO: Voltage monitor doesnt expose actual ADC vals yet
			static_cast<uint16_t>(voltageMonitor.vbatMv()),
...
int main() {
	...
	fsm.onStateChange([](FlightState oldState, FlightState newState) {
		logger.logStateChange(static_cast<uint8_t>(std::to_underlying(oldState)), static_cast<uint8_t>(std::to_underlying(newState)));
	...
	k_thread_create(&baroThread, baroStack, BARO_STACK_SIZE,
			baroThreadEntry, nullptr, nullptr, nullptr,
			BARO_PRIORITY, 0, K_NO_WAIT);

	k_thread_create(&voltageThread, voltageStack, VOLTAGE_STACK_SIZE,
			voltageThreadEntry, nullptr, nullptr, nullptr,
