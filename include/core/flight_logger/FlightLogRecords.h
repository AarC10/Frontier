...
inline constexpr size_t recordSize(RecordType type) {
	switch (type) {
	case RecordType::FLIGHT_HEADER:  return sizeof(FlightHeaderRecord);
	case RecordType::IMU:            return sizeof(ImuRecord);
	case RecordType::BARO:           return sizeof(BaroRecord);
	case RecordType::STATE_CHANGE:   return sizeof(StateChangeRecord);
	case RecordType::PYRO_EVENT:     return sizeof(PyroEventRecord);
	case RecordType::VOLTAGE:        return sizeof(VoltageRecord);
	case RecordType::TIMESTAMP_SYNC: return sizeof(TimestampSyncRecord);
	case RecordType::FLIGHT_END:     return sizeof(FlightEndRecord);
	}
	return 0;
}
 ...
 */
inline constexpr size_t recordSizeFromRaw(uint8_t rawType) {
	return recordSize(static_cast<RecordType>(rawType));
}
