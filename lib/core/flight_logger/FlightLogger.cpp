...
FlightLogger::FlightLogger(uint8_t partitionId)
	: partId(partitionId), writerStack{}, writerThread() {
...
int FlightLogger::init() {
	...
	LOG_INF("Write pointer recovered at offset 0x%08X (%u bytes used, %u free)",
	        writePtr, writePtr, partSize - writePtr);

	return 0;
}

int FlightLogger::startFlight(uint32_t flightId, uint16_t imuRateHz,
                              uint16_t baroRateHz) {
	...
	if (writerTid == nullptr) {
		writerTid = k_thread_create(
			&writerThread, writerStack, WRITER_STACK_SIZE,
			writerEntry, this, nullptr, nullptr,
			WRITER_PRIORITY, 0, K_NO_WAIT);
		k_thread_name_set(writerTid, "log_writer");
	}

	LOG_INF("Flight %u started (epoch=%u ms, writePtr=0x%08X)",
	        flightId, flightEpochMs, writePtr);
...
void FlightLogger::endFlight() {
	...
	LOG_INF("Flight ended (%u records, writePtr=0x%08X, %u dropped)",
	        flightRecordCount, writePtr,
...
void FlightLogger::logPyroEvent(uint8_t channel, FlightLog::PyroAction action,
                                uint16_t ilmRaw) {
...
void FlightLogger::logVoltage(uint16_t vbatRaw, uint16_t vccRaw,
                              uint16_t pyro1Raw, uint16_t pyro2Raw) {
	if (!recording.load(std::memory_order_relaxed)) return;
...
void FlightLogger::writerLoop() {
	...
	while (true) {
		...
		if (ret == 0) {
			...
			if (type == FlightLog::RecordType::STATE_CHANGE ||
				type == FlightLog::RecordType::PYRO_EVENT ||
		...
		if (!recording.load(std::memory_order_acquire) &&
			k_msgq_num_used_get(&msgq) == 0 && pageOffset > 0) {
			flushPage();
		}
	}
}
...
int FlightLogger::scanForWritePointer() {
	...
	while (offset < partSize) {
		...
		if (probe != FlightLog::RECORD_MAGIC) {
			LOG_WRN("Corruption at 0x%08X (0x%02X), rewinding to 0x%08X",
			        offset, probe, lastGoodOffset);
		...
		if (recSize == 0) {
			LOG_WRN("Unknown type 0x%02X at 0x%08X, rewinding to 0x%08X",
			        typeByte, offset, lastGoodOffset);
			offset = lastGoodOffset;
			break;
		}

		if (offset + recSize > partSize) {
			LOG_WRN("Record at 0x%08X overflows partition, rewinding to 0x%08X",
			        offset, lastGoodOffset);
			offset = lastGoodOffset;
			break;
		}
