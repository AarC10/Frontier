...
VoltageMonitor::VoltageMonitor(const device *vbatDev, const device *vccDev) : vccDev(vccDev), vbatDev(vbatDev) {}
...
bool VoltageMonitor::isVbatOk() const { return vbatMv() >= CONFIG_VBAT_MIN_MV; }

int VoltageMonitor::vccMv() const { return sensorValueToMilliVolts(vcc); }

int VoltageMonitor::vbatMv() const { return sensorValueToMilliVolts(vbat); }
